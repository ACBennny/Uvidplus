...
****************************************************************/
    ...
    let basicDislikesSliderCardBox;


    function preFetchingShowsFromDislikes()
    {
        basicDislikesSliderCardBox = document.querySelectorAll(".basic_dislikes_slider_card_box");

        if(basicDislikesSliderCardBox.length <= 0)
        {
            notification(`notifyBad` , `Failed to fetch Dislikes`);
            return;
        }
    ...
    async function fetchProfileDislikes() 
    {
        let selectedProfile = await getSelectedProfile();

        // Check if content of library is available
        if(((selectedProfile.prof_dislikes === undefined)))
        {
            errorLoadingMyListDislikes();
            return;
        }
        if(((selectedProfile.prof_dislikes.length === 0)))
        {
            return;
        }
        ...
        for(let i = 0; i < selectedProfile.prof_dislikes.length; i++)
        {
            const itemId = selectedProfile.prof_dislikes[i].ds_item;
        ...
        //
        basicDislikesSliderCardBox.forEach((box) => 
        {
